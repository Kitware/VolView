...
const refreshInputWarnings = () => {
  ...
  if (model) {
    for (const [parameterId, segmentationId] of Object.entries(
      ...
      // A plan only ever warns about what a single-labelmap input omits, and
      // planning costs a voxel sweep.
      if (acceptsMultipleLabelmaps(model, parameterId)) continue;
      const plan = planSegmentationInput(segmentationId, false);
