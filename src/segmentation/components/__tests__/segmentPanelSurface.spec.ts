...
//
// No panel says "segment group", "labelmap", "label value" or "layer" in
// user-visible text. Identifiers are out of scope, so the scan reads text nodes
// and the static attributes a user actually reads, never template expressions
// or component names.
// ---------------------------------------------------------------------------
...
describe('panel language', () => {
  it('keeps the segmentation panel free of group and storage words', () => {
    // These two must be present, so the scan is never vacuous; a renamed save
    // dialog simply drops out of the list.
