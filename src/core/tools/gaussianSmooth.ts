...
const CONFIDENCE_THRESHOLD = 0.1; // Keep/add label pixels above this confidence

export function gaussianSmoothLabelMap(
...
export function validateGaussianSmoothParams(
  params: GaussianSmoothParams
): string | null {
  if (params.sigma <= 0) {
  ...
  if (params.sigma > 10) {
    return 'Sigma too large (maximum 10)';
  }
  return null;
}
